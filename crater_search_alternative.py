...
db_basic_path = 'crater_db_final/cropped/'
...
def read_hdf_to_ram(idx, target_arr, filename):
...
def find_candidates(boxes, radii):
    ...
    for key in database.keys():
        ...
        avg += size
        if size == 0:
            print('warning: zero candidates found for crater ', str(key))
...
def check_tuples(boxes, radii):
    ...
    for count, (a, b) in enumerate(tuple_generator):
        ...
        crater_a_candidates_with_pair_id, crater_b_candidates_with_pair_id = check_single_tuple([boxes[a], boxes[b]],
                                                                                                [radii[a], radii[b]])
        crater_a_dict = {}
        crater_b_dict = {}
        for crater_id, pair_id in crater_a_candidates_with_pair_id:
            new_set = crater_a_dict.get(int(crater_id), set()) | {int(pair_id)}
            crater_a_dict[crater_id] = new_set
        for crater_id, pair_id in crater_b_candidates_with_pair_id:
            new_set = crater_b_dict.get(int(crater_id), set()) | {int(pair_id)}
            crater_b_dict[crater_id] = new_set
        add_to_db(a, crater_a_dict)
        add_to_db(b, crater_b_dict)
        print(str(count))
        if count % 10 == 0:
            check_db()
            if count >= 150:
...
def check_single_tuple(boxes, radii):
    ...
    return np.vstack((index[..., 0][pair_candidates], pair_candidates)).T, np.vstack(
        (index[..., 1][pair_candidates], pair_candidates)).T
...
def add_to_db(key, data):
    ...
    if entry is None or len(entry) == 0:
        ...
        intersection = set(entry.keys()) & set(data.keys())
        new_data = {}
        for k in intersection:
            new_data[k] = entry[k] | data[k]
        database[key] = new_data


def intersect_pair_ids_with_neighbours(key):
    n_keys = len(database)
    key_range = np.delete(np.arange(n_keys), key)
    pair_id_sets = database[key].values()
    id_collection = set()
    for id_set in pair_id_sets:
        id_collection = id_collection | id_set
    for crater in key_range:
        new_data = {}
        crater_dict = database[crater]
        for k in crater_dict.keys():
            intersect = id_collection & crater_dict[k]
            # if intersection isn't empty, add to new dictionary
            if bool(intersect):  new_data[k] = intersect

        database[crater] = new_data


# def update_pairings(key):
#     n_keys = len(database)
#     # tuple_generator = combos(n_keys, 2)
#     # for i, j in tuple_generator:
#     #
#     key_range = np.delete(np.arange(n_keys), key)
#     for k in key_range:
#         add_to_db(database, k, database[key])
#

def check_db():
    found = False

    for crater_dict in database.keys():
        intersect_pair_ids_with_neighbours(crater_dict)
        if len(database[crater_dict]) <= 6:
            print('Potential candidates out of ', str(len(database[crater_dict])), ' altogether: ')
            for c_id in crater_dict.keys():
                print(str(get_lunar_coordinates(c_id)))
            found = True
    return found
...
def search_size(size):
    ...
    result = k_vector.k_vector_search(True, y_a, y_b, pairs[..., 0], k_vector_radius, k_sort_radius, q_radius,
                                      m_radius)
    return result
